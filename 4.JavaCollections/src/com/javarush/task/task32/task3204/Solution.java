...
public class Solution {
    ...
    public static ByteArrayOutputStream getPassword() {
        ...
        for (int i = 0; i < passArr.length; i++) {
            ...
            passArr[i] = alphabet.substring(a, a + 1).charAt(0);
        }
        int n = (int) (Math.random() * 10);
        int nPos = (int) (Math.random() * 7);
        passArr[nPos] = numbers.substring(n, n + 1).charAt(0);

        int ac = (int) (Math.random() * 26);
        int acPos = (int) (Math.random() * 7);
        while (acPos == nPos) acPos = (int) (Math.random() * 7);
        passArr[acPos] = alphabetCapital.substring(ac, ac + 1).charAt(0);
